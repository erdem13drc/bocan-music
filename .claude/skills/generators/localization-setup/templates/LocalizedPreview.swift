            ...
            NavigationStack {
                List {
                    ForEach(self.locales, id: \.self) { locale in
                        Section(header: Text(self.localeName(for: locale))) {
                            self.content
                                .environment(\.locale, Locale(identifier: locale))
                                .environment(
                                    \.layoutDirection,
                                    self.isRTL(locale) ? .rightToLeft : .leftToRight
                                )
                        }
                    }
                }
    ...
    struct DynamicTypeLocalePreview<Content: View>: View {
        ...
        var body: some View {
            ForEach(self.sizes, id: \.self) { size in
                self.content
                    .environment(\.locale, Locale(identifier: self.locale))
                    .environment(\.dynamicTypeSize, size)
                    .previewDisplayName("\(self.locale) - \(String(describing: size))")
            }
        }
    }
