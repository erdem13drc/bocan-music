...
final class NotificationDelegate: NSObject, UNUserNotificationCenterDelegate, @unchecked Sendable {
    ...
    func userNotificationCenter(
        ...
        default:
            ...
            await self.handleCustomAction(actionIdentifier, payload: payload, response: response)
        }
    }
    ...
    private func handleCustomAction(
        ...
        case NotificationCategories.Action.reply.rawValue:
            ...
            if let textResponse = response as? UNTextInputNotificationResponse {
                await self.handleReply(text: textResponse.userText, payload: payload)
            }

        case NotificationCategories.Action.markRead.rawValue:
            await self.markAsRead(payload: payload)

        case NotificationCategories.Action.complete.rawValue:
            await self.completeTask(payload: payload)

        case NotificationCategories.Action.snooze.rawValue:
            await self.snoozeReminder(payload: payload)
