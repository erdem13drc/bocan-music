...
extension Repository {
    ...
    func fetch(sortBy: [SortDescriptor<T>] = []) async throws -> [T] {
        try await self.fetch(predicate: nil, sortBy: sortBy)
    }

    /// Fetch all items.
    func fetchAll() async throws -> [T] {
        try await self.fetch(predicate: nil, sortBy: [])
    }
    ...
    func deleteAll() async throws {
        try await self.deleteAll(predicate: nil)
    }
}
...
enum RepositoryError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case let .insertFailed(error):
            "Failed to insert data: \(error.localizedDescription)"
