...
final class StoreKitManager {
    ...
    func loadProducts() async {
        ...
        do {
            self.products = try await Product.products(for: Products.allProductIDs)
...
extension StoreKitManager {
    ...
    var monthlyProduct: Product? {
        self.products.first { $0.id == Products.monthlySubscription }
    }

    /// Yearly subscription product (if available).
    var yearlyProduct: Product? {
        self.products.first { $0.id == Products.yearlySubscription }
    }

    /// Lifetime purchase product (if available).
    var lifetimeProduct: Product? {
        self.products.first { $0.id == Products.lifetimePurchase }
    }
