...
struct APIConfiguration {
    ...
    func authorizationHeaders() -> [String: String] {
        ...
        // API key in header
        if let apiKey, case let .header(name) = apiKeyLocation {
            headers[name] = apiKey
        }
    ...
    func authorizationQueryItems() -> [URLQueryItem]? {
        guard let apiKey, case let .queryParam(name) = apiKeyLocation else {
            return nil
        }
...
extension EnvironmentValues {
    @Entry var apiClient: APIClient = URLSessionAPIClient(configuration: .production)
}
