...
extension ReviewPromptCondition {
    ...
    func isSatisfied(
        ...
        guard sessions >= self.minimumSessions else { return false }
        guard daysSinceInstall >= self.minimumDays else { return false }
        guard positiveActions >= self.minimumPositiveActions else { return false }

        // Check cooldown
        if let daysSinceLastPrompt {
            guard daysSinceLastPrompt >= self.cooldownDays else { return false }
        }
