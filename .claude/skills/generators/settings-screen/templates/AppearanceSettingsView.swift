    ...
    var body: some View {
        Form {
            Section("Theme") {
                Picker("Appearance", selection: self.$settings.appearance) {
                    ForEach(AppSettings.Appearance.allCases) { appearance in
                        HStack {
                            Image(systemName: self.iconForAppearance(appearance))
            ...
            Section("Effects") {
                ...
                Toggle("Haptic Feedback", isOn: self.$settings.hapticFeedback)
                Toggle("Sound Effects", isOn: self.$settings.soundEffects)
            }
...
// MARK: - App Icon Picker
    ...
    struct AppIconPickerView: View {
        ...
        var body: some View {
            ForEach(self.icons, id: \.0) { iconName, displayName in
                Button {
                    self.setAppIcon(iconName)
                } label: {
                    HStack(spacing: 12) {
                        // Icon preview image
            ...
            .onAppear {
                self.selectedIcon = self.currentIconName
            }
        }
        ...
        private func isSelected(_ iconName: String) -> Bool {
            if iconName == "AppIcon" {
                return self.selectedIcon == nil || self.selectedIcon == "AppIcon"
            }
            return self.selectedIcon == iconName
        }
